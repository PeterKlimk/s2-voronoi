...
use support::points::*;
use voronoi_mesh::{compute_with_report, UnitVec3Like, VoronoiConfig};
...
fn defect_fixture() -> Vec<voronoi_mesh::UnitVec3> {
    ...
    let c = SITE_CENTER;
    let mut fixture: Vec<voronoi_mesh::UnitVec3> = random_sphere_points(2_000_000, 1)
        .into_iter()
...
fn run_case(name: &str, points: &[voronoi_mesh::UnitVec3]) {
    voronoi_mesh::p5_shadow::reset();
    ...
    print!("{}", voronoi_mesh::p5_shadow::report());
}
...
fn probe_superset_band() {
    let run = |name: &str, points: &[voronoi_mesh::UnitVec3]| {
        voronoi_mesh::p5_shadow::reset();
        // Evaluate the exact predicate at every margin (not just < 1e-4), so a
        // large-margin disagreement cannot be missed.
        voronoi_mesh::p5_shadow::set_audit_cutoff(Some(10.0));
        let out = compute_with_report(points, VoronoiConfig::default()).expect(name);
        voronoi_mesh::p5_shadow::set_audit_cutoff(None);
        println!(
            "=== {name}: n={} defects={} ===",
            points.len(),
            out.report.unresolved_edge_pairs.len()
...
fn probe_superset_paired() {
    let run = |name: &str, points: &[voronoi_mesh::UnitVec3], cutoff: f64| {
        voronoi_mesh::p5_shadow::reset();
        voronoi_mesh::p5_shadow::paired_reset();
        voronoi_mesh::p5_shadow::set_pair_cutoff(cutoff);
        let out = compute_with_report(points, VoronoiConfig::default()).expect(name);
        voronoi_mesh::p5_shadow::set_pair_cutoff(0.0);
        println!(
            "=== {name}: n={} cutoff={cutoff:.0e} defects={} ===",
            points.len(),
            out.report.unresolved_edge_pairs.len()
        );
        print!("{}", voronoi_mesh::p5_shadow::paired_quad_report());
    };
...
fn probe_predicate_cost() {
    ...
    let time_one = |points: &[voronoi_mesh::UnitVec3], cutoff: f64| -> f64 {
        let mut best = f64::INFINITY;
        for _ in 0..3 {
            voronoi_mesh::p5_shadow::reset();
            voronoi_mesh::p5_shadow::set_audit_cutoff(Some(cutoff));
            let t = Instant::now();
...
fn probe_paired_audit() {
    let run = |name: &str, points: &[voronoi_mesh::UnitVec3], cutoff: f64| {
        ...
        print!("{}", voronoi_mesh::p5_shadow::paired_report());
    };
...
fn probe_defect_anatomy() {
    ...
    voronoi_mesh::p5_shadow::paired_reset();
    voronoi_mesh::p5_shadow::set_pair_cutoff(1e-3);
    ...
    print!("{}", voronoi_mesh::p5_shadow::paired_dump_involving(&site));
}
...
fn probe_two_pass_audit() {
    ...
    let run_two_pass = |name: &str, points: &[voronoi_mesh::UnitVec3], cutoff: f64| {
        // Pass 1: cutoff-based collection.
        voronoi_mesh::p5_shadow::paired_reset();
        voronoi_mesh::p5_shadow::set_pair_cutoff(cutoff);
        voronoi_mesh::p5_shadow::set_pair_key_filter(None);
        let out = compute_with_report(points, VoronoiConfig::default()).expect(name);
        voronoi_mesh::p5_shadow::set_pair_cutoff(0.0);
        let pass1 = voronoi_mesh::p5_shadow::paired_question_summaries();
        let keys: Vec<[u32; 4]> = pass1.iter().map(|&(k, ..)| k).collect();

        // Pass 2: same input, key-filtered, no margin censoring.
        voronoi_mesh::p5_shadow::paired_reset();
        voronoi_mesh::p5_shadow::set_pair_key_filter(Some(keys));
        let _ = compute_with_report(points, VoronoiConfig::default()).expect(name);
        voronoi_mesh::p5_shadow::set_pair_key_filter(None);
        let pass2 = voronoi_mesh::p5_shadow::paired_question_summaries();
    ...
    voronoi_mesh::p5_shadow::set_pair_cutoff(0.0);
    let site = [1790353u32, 2327897, 2902347, 2992988];
    let keys: Vec<[u32; 4]> = voronoi_mesh::p5_shadow::paired_question_summaries()
        .into_iter()
...
fn probe_eps_cert_sweep() {
    ...
    for pad in [None, Some(1e-6f64), Some(1e-5), Some(1e-4)] {
        voronoi_mesh::p5_shadow::set_term_pad_override(pad);
        println!("=== pad={pad:?} ===");
        for (name, points) in [
    ...
    voronoi_mesh::p5_shadow::set_term_pad_override(None);
}
...
fn probe_tie_rule_sweep() {
    ...
    for eps in [None, Some(0.0f64), Some(1e-14), Some(1e-13)] {
        voronoi_mesh::p5_shadow::set_clip_eps_override(eps);
    ...
    voronoi_mesh::p5_shadow::set_clip_eps_override(Some(0.0));
    ...
    voronoi_mesh::p5_shadow::set_clip_eps_override(None);
}
...
fn probe_45m_quad_anatomy() {
    ...
    for eps in [None, Some(0.0f64)] {
        voronoi_mesh::p5_shadow::set_clip_eps_override(eps);
        voronoi_mesh::p5_shadow::paired_reset();
        voronoi_mesh::p5_shadow::set_pair_cutoff(1e-4);
        ...
        print!("{}", voronoi_mesh::p5_shadow::paired_dump_involving(&ids));
    }
    voronoi_mesh::p5_shadow::set_clip_eps_override(None);
}
...
fn probe_escalation_band_sweep() {
    ...
    for factor in [0.0f64, 64.0] {
        voronoi_mesh::p5_shadow::set_escalation_factor_override(Some(factor));
    ...
    voronoi_mesh::p5_shadow::set_escalation_factor_override(None);
}
