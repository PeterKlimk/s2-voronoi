...
fn assert_fuzz_strictly_valid(name: &str, points: &[support::points::TestPoint]) {
...
fn test_weld_ulp_cluster_strictly_valid_by_default() {
    ...
    let p = points[500];
    points.push(support::points::TestPoint::new(p.x.next_up(), p.y, p.z));
    points.push(support::points::TestPoint::new(p.x, p.y.next_up(), p.z));
    points.push(support::points::TestPoint::new(p.x, p.y, p.z.next_up()));
    points.push(support::points::TestPoint::new(
        p.x.next_down(),
        p.y.next_up(),
        p.z,
    ));
...
fn test_weld_seam_ulp_pairs_strictly_valid_by_default() {
    ...
    let mut seam_points: Vec<support::points::TestPoint> = Vec::new();
    for sx in [-1.0f32, 1.0] {
        for sy in [-1.0f32, 1.0] {
            for sz in [-1.0f32, 1.0] {
                seam_points.push(support::points::TestPoint::new(
                    sx * inv3,
                    sy * inv3,
                    sz * inv3,
                ));
    ...
    for s in [-1.0f32, 1.0] {
        seam_points.push(support::points::TestPoint::new(s * inv2, s * inv2, 0.0));
        seam_points.push(support::points::TestPoint::new(s, 0.0, 0.0));
        seam_points.push(support::points::TestPoint::new(0.0, 0.0, s));
    }
    let num_pairs = seam_points.len();
    for p in seam_points {
        let twin = support::points::TestPoint::new(
...
fn test_nan_input_rejected_with_index() {
    ...
    points[437] = support::points::TestPoint::new(f32::NAN, 0.5, 0.5);
...
fn test_infinite_input_rejected_with_index() {
    let mut points = random_sphere_points(1_000, 17);
    points[12] = support::points::TestPoint::new(0.0, f32::INFINITY, 0.0);
...
fn test_sub_weld_cluster_without_welding_is_degenerate_input() {
    ...
    for i in 0..50 {
        ...
        while placed < 4 && tries < 200 {
            ...
            let q = support::points::TestPoint::new(q64.x as f32, q64.y as f32, q64.z as f32);
