...
- **2026-08-04 evidence:** On native 16-worker `mega 500k`, nine cells examined 400k--499k
  candidates each; their final no-progress tails reached 91.0--100% and consumed about 2.2 seconds
  of aggregate worker time. The worst cell spent 322.8 ms (148.2 ms query, 174.6 ms clipping).
  `great-circle 200k` showed a broad version of the same regime: 5,835 cells examined at least
  1,024 candidates, with many reaching nearly all 200k generators.
- **Rejected handoff:** A hull of the target plus its nearest generators is not a correct local
  replacement for dense-cluster boundary cells. On the worst `mega` cell, hulls through k=2,048
...
- **Scope:** retain supported outputs, robustness, accepted-input behavior, and the packed/grid
  ...
  the trade. Judge complexity at the general-algorithm and module-boundary level: generated or
  tightly siloed kernels are not charged line-for-line when their interface and generator remain
  understandable and their specialization materially improves performance.
...
- **Restored the siloed 24-key network:** `a42980f` experimentally replaced both 24-key kernels
  ...
  7--8 KiB linked text. The isolated N=20--24 operation became 81--108% slower and ordinary builds
  lost roughly 0.6--0.9%. `9bc2f9c` restored the network after reviewing the correct maintenance
  boundary: its 469 generated Rust lines and compact 124-line schedule are owned by the existing
  generator and do not complicate the general Voronoi algorithm. The replacement instead added a
