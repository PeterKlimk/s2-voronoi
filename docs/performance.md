...
Ordinary and edge-check clipping share one source copy of the small-polygon specialization
dispatcher. Earlier toolchain measurements favored forced outlining because duplicating the table
cost about 6.0 KiB of native text and 7.6 KiB of portable text. A current-toolchain null instead
removed forced outlining from the dispatcher, both wrappers, and six large or cold helpers while
retaining the two resolution scans. In nine paired 500k single-threaded runs, Fibonacci and uniform
cycles fell 0.89% and 0.97%; uniform instructions fell 0.47%, while Fibonacci instructions were
effectively flat (-0.07%). Seven paired 100k mega runs also improved cycles by 0.63%. Keep the
shared source shape, but let the compiler choose these code boundaries rather than encoding the
old placement result in attributes.
...
A follow-up tested replacing the separate full-sort 24-key network with a 16-key block, a short
suffix sort, and an in-place backward merge. It removed 581 implementation/data lines and about
7--8 KiB linked text, but made the isolated N=20--24 operation 81--108% slower and raised ordinary
whole-build wall time about 0.6--0.9%. The experiment was restored. The apparent source reduction
was also misleading for maintainability: 469 removed Rust lines were generated, the compact
124-line comparator schedule was already owned by the generator, and the replacement introduced a
second handwritten sorting strategy and merge invariants. Generated, tightly siloed kernels should
be judged by their generator and interface rather than raw expansion size; retain the 24-key
network.
...
- **Shared packed stage-key storage.** Each packed query reuses its exhausted chunk-zero key
  ...
  0.3%, and branches about 0.7--0.8%. A later quiet-host counter retest was neutral/favorable, while
  wall-clock ratios remained mixed across 1M and 4M. Cache misses rose 4.0%/1.1%, so preserve this
  primarily as a measured memory-envelope and retired-work result rather than assuming every cache
  counter or wall-time sample improves.
