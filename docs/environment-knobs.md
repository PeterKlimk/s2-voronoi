...
| `VORONOI_MESH_BIN_COUNT` | supported tuning | `live_dedup::binning::target_bin_count_with_override`, once per computation | Explicit integer shard target, clamped to `[6, 96]`, quantized to the cube-face layout, and disabling adaptive refinement. Without an override, the default is 2x workers through 8 workers and a 96-bin coarse layout at 9+; a severe, predictably reducible coarse population imbalance may refine to 216 bins. Tests and benchmark invocations may override it. |
...
| `VORONOI_MESH_TIMING_KV` | instrumentation | `timing::real::PhaseTimings::report` | Presence emits machine-readable coarse pipeline wall timing when the `timing` feature is enabled. |
| `VORONOI_MESH_TELEMETRY_KV` | instrumentation | `telemetry::real` report | Presence emits machine-readable algorithmic work counters when the `telemetry` feature is enabled. |
