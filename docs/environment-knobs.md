...
| `VORONOI_MESH_BIN_COUNT` | supported tuning | `live_dedup::binning::target_bin_count`, once per computation | Integer shard target, clamped to `[6, 96]`; defaults to 2x workers below 12 and the full 96-bin cube layout at 12+, then is quantized to the cube-face layout. Tests and benchmark invocations may override it. |
