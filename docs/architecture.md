...
- `timing/` contains coarse whole-pipeline wall timing, while `telemetry/` contains real and zero-sized algorithmic-counter backends. `quality.rs` is the `tools`-only quality
  surface. `sort.rs` is the small-sort facade used by packed production selection and microbench
  code.
