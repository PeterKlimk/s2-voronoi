use super::types::{ClipResult, HalfPlane, PolyBuffer};

mod bitmask;
mod output;
mod small;

use bitmask::clip_bitmask;
use small::clip_small_ptr;

/// Intersection parameters for two strict-boundary transitions.
#[inline(always)]
fn lerp_t_pair(d0: f64, d1: f64, d2: f64, d3: f64) -> (f64, f64) {
    (d0 / (d0 - d1), d2 / (d2 - d3))
}

/// Dispatch clipping to the best specialization for the current polygon size.
///
/// This is the shared match table used by both `clip_convex` and `clip_convex_edgecheck`.
///
/// - N=4,8: `% N` is cheap (bitmask), so the plain ptr variant tends to win.
/// - N=3,5,6,7: avoid `% N` in the hot loop via the ptr_d variant.
///
/// We `match` on N first, then branch on bounded/unbounded per arm.
// Keep one source copy of the specialization table and let the compiler choose
// the dispatcher and wrapper boundaries. See the shared clip-dispatch result
// in docs/performance.md.
fn dispatch_clip(poly: &PolyBuffer, hp: &HalfPlane, out: &mut PolyBuffer) -> ClipResult {
    let n = poly.len;
    let has_bounding_ref = poly.has_bounding_ref;
    match n {
        3 => {
            if has_bounding_ref {
                clip_small_ptr::<3, true>(poly, hp, out)
            } else {
                clip_small_ptr::<3, false>(poly, hp, out)
            }
        }
        4 => {
            if has_bounding_ref {
                clip_small_ptr::<4, true>(poly, hp, out)
            } else {
                clip_small_ptr::<4, false>(poly, hp, out)
            }
        }
        5 => {
            if has_bounding_ref {
                clip_small_ptr::<5, true>(poly, hp, out)
            } else {
                clip_small_ptr::<5, false>(poly, hp, out)
            }
        }
        6 => {
            if has_bounding_ref {
                clip_small_ptr::<6, true>(poly, hp, out)
            } else {
                clip_small_ptr::<6, false>(poly, hp, out)
            }
        }
        7 => {
            if has_bounding_ref {
                clip_small_ptr::<7, true>(poly, hp, out)
            } else {
                clip_small_ptr::<7, false>(poly, hp, out)
            }
        }
        8 => {
            if has_bounding_ref {
                clip_small_ptr::<8, true>(poly, hp, out)
            } else {
                clip_small_ptr::<8, false>(poly, hp, out)
            }
        }
        _ => clip_bitmask(poly, hp, out),
    }
}

/// Clip a convex polygon by a half-plane.
pub(crate) fn clip_convex(poly: &PolyBuffer, hp: &HalfPlane, out: &mut PolyBuffer) -> ClipResult {
    let n = poly.len;

    debug_assert!(n >= 3, "clip_convex expects poly.len >= 3, got {}", n);

    let has_bounding_ref = poly.has_bounding_ref;
    // Early-unchanged is only relevant for bounded polys. Empirically, it is very rare for small N
    // (e.g. triangles), so we skip the check below a small-N threshold to save overhead.
    const EARLY_UNCHANGED_MIN_N: usize = 5;
    if !has_bounding_ref && n >= EARLY_UNCHANGED_MIN_N {
        let max_r2 = poly.max_r2;
        debug_assert!(max_r2 >= 0.0, "polygon max radius squared is invalid");
        let t = hp.c;
        if t >= 0.0 && t * t >= hp.ab2 * max_r2 {
            return ClipResult::Unchanged;
        }
    }

    dispatch_clip(poly, hp, out)
}

/// Clip a convex polygon by a half-plane, skipping the early-unchanged bounding check.
///
/// This is intended for edgecheck-derived seed constraints where we expect the half-plane to be
/// active and want to avoid extra branchy prechecks.
pub(crate) fn clip_convex_edgecheck(
    poly: &PolyBuffer,
    hp: &HalfPlane,
    out: &mut PolyBuffer,
) -> ClipResult {
    debug_assert!(
        poly.len >= 3,
        "clip_convex expects poly.len >= 3, got {}",
        poly.len
    );

    dispatch_clip(poly, hp, out)
}

#[cfg(feature = "microbench")]
pub(crate) use small::clip_convex_small_bool;

#[cfg(test)]
mod tests {
    use super::lerp_t_pair;

    #[test]
    fn strict_lerp_pair_is_raw_division_bit_for_bit() {
        let inputs = (3.0_f64, -2.0_f64, -7.0_f64, 5.0_f64);
        let expected = (
            inputs.0 / (inputs.0 - inputs.1),
            inputs.2 / (inputs.2 - inputs.3),
        );
        let actual = lerp_t_pair(inputs.0, inputs.1, inputs.2, inputs.3);
        assert_eq!(actual.0.to_bits(), expected.0.to_bits());
        assert_eq!(actual.1.to_bits(), expected.1.to_bits());
    }
}
