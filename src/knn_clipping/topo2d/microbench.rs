...
pub fn run_clip_convex_microbench() {
    use super::clippers::{
        ...
        clip_convex_small_bool_out_idx_ptr_dyn8,
        clip_convex_small_bool_out_idx_ptr_b, clip_convex_small_bool_out_idx_ptr_c,
        clip_convex_small_bool_out_idx_ptr_d, clip_convex_small_bool_out_idx_ptr_d_split,
        clip_convex_small_bool_out_idx_ptr_split,
        clip_convex_small_bool_out_idx_ptr_range_d,
    ...
    fn run_for<const N: usize>(target: Duration, samples: usize, hp_pool_len: usize) {
        ...
        let mut out_out_idx_ptr_d_split = PolyBuffer::new();
        let mut out_out_idx_ptr_split = PolyBuffer::new();
        let mut out_out_idx_ptr_dyn8 = PolyBuffer::new();
        ...
        assert!(matches!(
            clip_convex_small_bool_out_idx_range::<N>(&poly, &hps_changed[0], &mut out_out_idx_range),
            ClipResult::Changed
        ));
        assert!(matches!(
        ...
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_d_split::<N>(
                &poly,
                &hps_changed[0],
                &mut out_out_idx_ptr_d_split
            ),
            ClipResult::Changed
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_split::<N>(&poly, &hps_changed[0], &mut out_out_idx_ptr_split),
            ClipResult::Changed
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_dyn8(&poly, &hps_changed[0], &mut out_out_idx_ptr_dyn8),
            ClipResult::Changed
        ));
        assert!(matches!(
        ...
        assert!(matches!(
            clip_convex_small_bool_out_idx_range::<N>(&poly, &hps_unchanged[0], &mut out_out_idx_range),
            ClipResult::Unchanged
        ));
        assert!(matches!(
        ...
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_split::<N>(&poly, &hps_unchanged[0], &mut out_out_idx_ptr_split),
            ClipResult::Unchanged
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_dyn8(&poly, &hps_unchanged[0], &mut out_out_idx_ptr_dyn8),
            ClipResult::Unchanged
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_range_d::<N>(
        ...
        for (i, hp) in hps_changed.iter().take(64).enumerate() {
            ...
            assert!(
                ...
                "out_idx_ptr_d_split unexpectedly not Changed (N={N}, i={i})"
            );
            assert_same_poly("out_idx_ptr_d_split", baseline, &out_out_idx_ptr_d_split);

            assert!(
                matches!(
                    clip_convex_small_bool_out_idx_ptr_split::<N>(&poly, hp, &mut out_out_idx_ptr_split),
                    ClipResult::Changed
                ),
                "out_idx_ptr_split unexpectedly not Changed (N={N}, i={i})"
            );
            assert_same_poly("out_idx_ptr_split", baseline, &out_out_idx_ptr_split);

            assert!(
                matches!(
                    clip_convex_small_bool_out_idx_ptr_dyn8(&poly, hp, &mut out_out_idx_ptr_dyn8),
                    ClipResult::Changed
                ),
                "out_idx_ptr_dyn8 unexpectedly not Changed (N={N}, i={i})"
            );
            assert_same_poly("out_idx_ptr_dyn8", baseline, &out_out_idx_ptr_dyn8);
        ...
        bench_ns_per_call("out_idx_range mixed", target, samples, 1, |iters| {
            let poly = black_box(&poly);
            let hps = black_box(hps_changed.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_out_idx_range);
            let mut s = 0x1234_5678_9ABC_DEF0u64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_range::<N>(poly, hp, out);
                black_box(r);
            }
        });
        ...
        bench_ns_per_call("out_idx_ptr_d_split mixed", target, samples, 1, |iters| {
            let poly = black_box(&poly);
            let hps = black_box(hps_changed.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_out_idx_ptr_d_split);
            let mut s = 0x1234_5678_9ABC_DEF0u64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_ptr_d_split::<N>(poly, hp, out);
                black_box(r);
            }
        });
        bench_ns_per_call("out_idx_ptr_split mixed", target, samples, 1, |iters| {
            let poly = black_box(&poly);
            let hps = black_box(hps_changed.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_out_idx_ptr_split);
            let mut s = 0x1234_5678_9ABC_DEF0u64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_ptr_split::<N>(poly, hp, out);
                black_box(r);
            }
        });
        bench_ns_per_call("out_idx_ptr_dyn8 mixed", target, samples, 1, |iters| {
            let poly = black_box(&poly);
            let hps = black_box(hps_changed.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_out_idx_ptr_dyn8);
            let mut s = 0x1234_5678_9ABC_DEF0u64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_ptr_dyn8(poly, hp, out);
                black_box(r);
            }
        });
        ...
        bench_ns_per_call("out_idx_range unchanged", target, samples, 1, |iters| {
            let poly = black_box(&poly);
            let hps = black_box(hps_unchanged.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_out_idx_range);
        ...
        bench_ns_per_call("out_idx_ptr_d_split unchanged", target, samples, 1, |iters| {
            let poly = black_box(&poly);
            let hps = black_box(hps_unchanged.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_out_idx_ptr_d_split);
        ...
        bench_ns_per_call("out_idx_ptr_split unchanged", target, samples, 1, |iters| {
            let poly = black_box(&poly);
            let hps = black_box(hps_unchanged.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_out_idx_ptr_split);
        ...
        bench_ns_per_call("out_idx_ptr_dyn8 unchanged", target, samples, 1, |iters| {
            let poly = black_box(&poly);
            let hps = black_box(hps_unchanged.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_out_idx_ptr_dyn8);
        ...
        let mut out_u_out_idx_ptr_d = PolyBuffer::new();
        let mut out_u_out_idx_ptr_d_split = PolyBuffer::new();
        let mut out_u_out_idx_ptr_split = PolyBuffer::new();
        let mut out_u_out_idx_ptr_dyn8 = PolyBuffer::new();
        ...
        assert!(matches!(
            clip_convex_small_bool_out_idx_range::<N>(&poly_u, &hps_keep[0], &mut out_u_out_idx_range),
            ClipResult::Changed
        ));
        assert!(matches!(
        ...
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_d::<N>(&poly_u, &hps_keep[0], &mut out_u_out_idx_ptr_d),
            ClipResult::Changed
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_d_split::<N>(
                &poly_u,
                &hps_keep[0],
                &mut out_u_out_idx_ptr_d_split
            ),
            ClipResult::Changed
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_split::<N>(&poly_u, &hps_keep[0], &mut out_u_out_idx_ptr_split),
            ClipResult::Changed
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_dyn8(&poly_u, &hps_keep[0], &mut out_u_out_idx_ptr_dyn8),
            ClipResult::Changed
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_range_d::<N>(
        ...
        out_u_out_idx_range.len = 13;
        out_u_out_idx_range.us[0] = 123.0;
        ...
        assert!(matches!(
            clip_convex_small_bool_out_idx_range::<N>(&poly_u, &hps_u_unchanged[0], &mut out_u_out_idx_range),
            ClipResult::Unchanged
        ));
        assert!(matches!(
        ...
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_d::<N>(
                &poly_u,
                &hps_u_unchanged[0],
                &mut out_u_out_idx_ptr_d
            ),
            ClipResult::Unchanged
        ));
        assert!(matches!(
        ...
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_split::<N>(&poly_u, &hps_u_unchanged[0], &mut out_u_out_idx_ptr_split),
            ClipResult::Unchanged
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_dyn8(&poly_u, &hps_u_unchanged[0], &mut out_u_out_idx_ptr_dyn8),
            ClipResult::Unchanged
        ));
        assert!(matches!(
            clip_convex_small_bool_out_idx_ptr_range_d::<N>(
        ...
        for (i, hp) in hps_keep.iter().take(64).enumerate() {
            ...
            assert!(
                matches!(
                    clip_convex_small_bool_out_idx_ptr_d::<N>(&poly_u, hp, &mut out_u_out_idx_ptr_d),
                    ClipResult::Changed
                ),
                "out_idx_ptr_d unexpectedly not Changed (unbounded N={N}, i={i})"
            );
            assert_same_poly("unbounded out_idx_ptr_d", baseline, &out_u_out_idx_ptr_d);
            ...
            assert!(
                ...
                "out_idx_ptr_d_split unexpectedly not Changed (unbounded N={N}, i={i})"
            );
            assert_same_poly(
                "unbounded out_idx_ptr_d_split",
                baseline,
                &out_u_out_idx_ptr_d_split,
            );

            assert!(
                matches!(
                    clip_convex_small_bool_out_idx_ptr_split::<N>(&poly_u, hp, &mut out_u_out_idx_ptr_split),
                    ClipResult::Changed
                ),
                "out_idx_ptr_split unexpectedly not Changed (unbounded N={N}, i={i})"
            );
            assert_same_poly("unbounded out_idx_ptr_split", baseline, &out_u_out_idx_ptr_split);

            assert!(
                matches!(
                    clip_convex_small_bool_out_idx_ptr_dyn8(&poly_u, hp, &mut out_u_out_idx_ptr_dyn8),
                    ClipResult::Changed
                ),
                "out_idx_ptr_dyn8 unexpectedly not Changed (unbounded N={N}, i={i})"
            );
            assert_same_poly("unbounded out_idx_ptr_dyn8", baseline, &out_u_out_idx_ptr_dyn8);
        ...
        bench_ns_per_call("out_idx_range keep_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_keep.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_range);
            let mut s = 0xA11C_E11E_A11C_E11Eu64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_range::<N>(poly, hp, out);
        ...
        bench_ns_per_call("out_idx_ptr_d keep_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_keep.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_d);
            let mut s = 0xA11C_E11E_A11C_E11Eu64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_ptr_d::<N>(poly, hp, out);
        ...
        bench_ns_per_call("out_idx_ptr_d_split keep_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_keep.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_d_split);
            let mut s = 0xA11C_E11E_A11C_E11Eu64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_ptr_d_split::<N>(poly, hp, out);
        ...
        bench_ns_per_call("out_idx_ptr_split keep_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_keep.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_split);
            let mut s = 0xA11C_E11E_A11C_E11Eu64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_ptr_split::<N>(poly, hp, out);
        ...
        bench_ns_per_call("out_idx_ptr_dyn8 keep_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_keep.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_dyn8);
            let mut s = 0xA11C_E11E_A11C_E11Eu64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_ptr_dyn8(poly, hp, out);
        ...
        bench_ns_per_call("out_idx_ptr_range_d keep_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_keep.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_range_d);
            let mut s = 0xA11C_E11E_A11C_E11Eu64;
            for _ in 0..iters {
                let hp = &hps[next_idx(&mut s, hp_mask)];
                let r = clip_convex_small_bool_out_idx_ptr_range_d::<N>(poly, hp, out);
        ...
        bench_ns_per_call("out_idx_range drop_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_drop.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_range);
        ...
        bench_ns_per_call("out_idx_ptr_d drop_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_drop.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_d);
        ...
        bench_ns_per_call("out_idx_ptr_d_split drop_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_drop.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_d_split);
        ...
        bench_ns_per_call("out_idx_ptr_split drop_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_drop.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_split);
        ...
        bench_ns_per_call("out_idx_ptr_dyn8 drop_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_drop.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_dyn8);
        ...
        bench_ns_per_call("out_idx_ptr_range_d drop_bounding", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_drop.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_range_d);
        ...
        bench_ns_per_call("out_idx_ptr_d unchanged", target, samples, 1, |iters| {
            let poly = black_box(&poly_u);
            let hps = black_box(hps_u_unchanged.as_slice());
            let hp_mask = hps.len() - 1;
            let out = black_box(&mut out_u_out_idx_ptr_d);
...
pub fn run_batch_clip_microbench() {
    ...
    enum HpPoolMode {
        /// Old behavior: random offset in roughly `[-norm, +norm]`.
        ///
        /// This often makes the polygon go empty after only a few planes (serial benefits
        /// a lot from early-break; batch pays upfront classification cost).
        Random,
        /// Planes are guaranteed to include the origin (`c >= 0`), so the intersection
        /// stays non-empty for polygons that contain the origin (the typical Voronoi use-case).
        OriginInside,
    }

    fn parse_hp_pool_mode() -> HpPoolMode {
        match std::env::var("S2_VORONOI_BATCH_HP_MODE")
            .ok()
            .as_deref()
            .unwrap_or("origin")
        {
            "random" => HpPoolMode::Random,
            "origin" | "origin_inside" | "nonempty" => HpPoolMode::OriginInside,
            other => {
                eprintln!(
                    "warning: unknown S2_VORONOI_BATCH_HP_MODE={other:?}; using \"origin\""
                );
                HpPoolMode::OriginInside
            }
        }
    }
    ...
    // Generate a pool of half-planes for the batch microbench.
    fn build_hp_pool<const N: usize>(poly: &PolyBuffer, mode: HpPoolMode) -> Vec<HalfPlane> {
        let full_mask_val: u8 = ((1u16 << N) - 1) as u8;

        let mut hps = Vec::with_capacity(64);
        let mut rng = 0x1234_5678_9ABC_DEF0u64 ^ (N as u64).wrapping_mul(0x9E37_79B9_7F4A_7C15);

        // Rejection-sample to keep the pool "interesting" at the starting polygon:
        // - avoid trivially unchanged planes
        // - (for `origin`) ensure the intersection cannot go empty
        let mut attempts: u32 = 0;
        while hps.len() < 64 {
            attempts += 1;
            if attempts > 5_000_000 {
                panic!("build_hp_pool: too many attempts (mode={mode:?}, N={N})");
            }
            ...
            let t = (r16 as f64) / (u16::MAX as f64);
            let cc = match mode {
                HpPoolMode::Random => (t - 0.5) * norm * 2.0,
                HpPoolMode::OriginInside => t * norm * 0.95,
            };

            let hp = HalfPlane::new_unnormalized(a, b, cc, hps.len());
            if matches!(mode, HpPoolMode::OriginInside) && hp.c < 0.0 {
                continue;
            }

            let m = mask_for::<N>(poly, &hp);
            if m == full_mask_val || m == 0 {
                continue;
            }
    ...
    let hp_mode = parse_hp_pool_mode();
    eprintln!("hp_mode: {hp_mode:?}\n");

    for n in [3, 4, 5, 6, 7, 8] {
        eprintln!("\n--- N = {n} ---");

        match n {
            3 => run_batch_bench::<3>(target_ms, samples, hp_mode),
            4 => run_batch_bench::<4>(target_ms, samples, hp_mode),
            5 => run_batch_bench::<5>(target_ms, samples, hp_mode),
            6 => run_batch_bench::<6>(target_ms, samples, hp_mode),
            7 => run_batch_bench::<7>(target_ms, samples, hp_mode),
            8 => run_batch_bench::<8>(target_ms, samples, hp_mode),
            _ => unreachable!(),
        }
    }
    ...
    fn run_batch_bench<const N: usize>(target_ms: u64, samples: usize, hp_mode: HpPoolMode) {
        use super::batch_clip::{clip_batch_with_scratch as clip_batch4, HpBatch as HpBatch4};
        use super::batch_clip8::{clip_batch_with_scratch as clip_batch8, HpBatch8};

        let poly = make_regular_poly::<N>(1.0);
        let hp_pool = build_hp_pool::<N>(&poly, hp_mode);

        // Prebuild batches so the benchmark measures `clip_batch` rather than the
        // gather/construction cost.
        let mut batches4_0: Vec<HpBatch4> = Vec::with_capacity(56);
        let mut batches4_1: Vec<HpBatch4> = Vec::with_capacity(56);
        let mut batches8: Vec<HpBatch8> = Vec::with_capacity(56);
        for start in 0..56usize {
            batches4_0.push(HpBatch4::from_array([
                hp_pool[start],
                hp_pool[start + 1],
                hp_pool[start + 2],
                hp_pool[start + 3],
            ]));
            batches4_1.push(HpBatch4::from_array([
                hp_pool[start + 4],
                hp_pool[start + 5],
                hp_pool[start + 6],
                hp_pool[start + 7],
            ]));
            batches8.push(HpBatch8::from_array([
                hp_pool[start],
                hp_pool[start + 1],
                hp_pool[start + 2],
                hp_pool[start + 3],
        ...
        // often polygons become empty before all planes are applied.
        {
            ...
            for iter in 0..iters {
                ...
                // batch4
                let r0 = clip_batch4(&poly, &batches4_0[start], &mut out2, &mut out1);
                ...
                if matches!(r0, ClipResult::Changed) && out2.len == 0 {
                    ...
                    let r1 = clip_batch4(input, &batches4_1[start], &mut out3, &mut out1);
                ...
                // batch8
                let r = clip_batch8(&poly, &batches8[start], &mut out2, &mut out1);
        ...
        bench_ns_per_call("batch4 (2×4 at once)", target, samples, |iters| {
            let poly = black_box(&poly);
            let batches4_0 = black_box(batches4_0.as_slice());
            let batches4_1 = black_box(batches4_1.as_slice());
            let scratch = black_box(&mut out1);
            let out2 = black_box(&mut out2);
            ...
            for _ in 0..iters {
                ...
                // First batch of 4
                let r0 = clip_batch4(current_poly, &batch0, out2, scratch);
                ...
                // Second batch of 4
                let batch1 = &batches4_1[start];
                let r1 = clip_batch4(current_poly, batch1, out3, scratch);
        ...
        bench_ns_per_call("batch8 (1×8 at once)", target, samples, |iters| {
            let poly = black_box(&poly);
            let batches8 = black_box(batches8.as_slice());
            let scratch = black_box(&mut out1);
            let out2 = black_box(&mut out2);
            let mut s = 0u64;
            ...
            for _ in 0..iters {
                ...
                let r = clip_batch8(poly, &batches8[start], out2, scratch);
                black_box(r);
                if matches!(r, ClipResult::Changed) {
                    consume_poly(out2);
                } else {
