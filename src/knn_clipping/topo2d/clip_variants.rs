...
/// Baseline small-bool classification + transition scan, but uses a modulo-free vertex copy.
#[cfg_attr(feature = "profiling", inline(never))]
#[cfg(any(test, feature = "microbench"))]
pub(super) fn clip_convex_small_bool_range_copy<const N: usize>(
    poly: &PolyBuffer,
    hp: &HalfPlane,
    out: &mut PolyBuffer,
) -> ClipResult {
...
/// Like `clip_convex_small_bool_range_copy`, but hoists both intersections before copying vertices.
#[cfg_attr(feature = "profiling", inline(never))]
#[cfg(any(test, feature = "microbench"))]
pub(super) fn clip_convex_small_bool_range_copy_hoist<const N: usize>(
    poly: &PolyBuffer,
    hp: &HalfPlane,
    out: &mut PolyBuffer,
) -> ClipResult {
...
/// Single-pass classification with transitions captured only once we know the polygon is mixed
/// (we've seen at least one inside and one outside).
#[cfg_attr(feature = "profiling", inline(never))]
#[cfg(any(test, feature = "microbench"))]
pub(super) fn clip_convex_small_bool_stream_guarded<const N: usize>(
    poly: &PolyBuffer,
    hp: &HalfPlane,
    out: &mut PolyBuffer,
) -> ClipResult {
    ...
    let mut have_inside = prev_inside;
    let mut have_outside = !prev_inside;

    // Captured transitions (prev -> cur) and the corresponding distances.
    let mut entry: Option<(usize, usize, f64, f64)> = None;
    let mut exit: Option<(usize, usize, f64, f64)> = None;
    ...
    unsafe {
        for i in 0..N {
            ...
            // Only start looking for entry/exit transitions once the polygon is known mixed.
            if have_inside && have_outside {
                if entry.is_none() && !prev_inside && inside {
                    entry = Some((prev_idx, i, d_prev, d));
                } else if exit.is_none() && prev_inside && !inside {
                    exit = Some((prev_idx, i, d_prev, d));
...
/// Baseline classification + transition scan, but avoids `out.push_raw`/`out.len` traffic by using
/// a local output index and unchecked stores.
#[cfg_attr(feature = "profiling", inline(never))]
#[cfg(any(test, feature = "microbench"))]
pub(super) fn clip_convex_small_bool_out_idx<const N: usize>(
    poly: &PolyBuffer,
    hp: &HalfPlane,
    out: &mut PolyBuffer,
) -> ClipResult {
