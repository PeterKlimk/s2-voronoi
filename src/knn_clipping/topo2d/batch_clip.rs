...
impl HpBatch {
    ...
    pub unsafe fn from_slice_unchecked(hps: &[HalfPlane]) -> Self {
        ...
        let mut a_arr = [0.0f64; 4];
        let mut b_arr = [0.0f64; 4];
        let mut c_arr = [0.0f64; 4];
        let mut ab2_arr = [0.0f64; 4];
        let mut eps_arr = [0.0f64; 4];
        ...
        Self {
            a: f64x4::from_array(a_arr),
            b: f64x4::from_array(b_arr),
            c: f64x4::from_array(c_arr),
            ab2: f64x4::from_array(ab2_arr),
            eps: f64x4::from_array(eps_arr),
            a_arr,
            b_arr,
            c_arr,
            ab2_arr,
...
#[inline]
fn classify_lanes_only(poly: &PolyBuffer, batch: &HpBatch) -> [u64; 4] {
    let n = poly.len;
