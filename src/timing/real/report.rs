    ...
    pub(crate) fn report(&self, n: usize) {
        ...
        if std::env::var_os("VORONOI_MESH_TIMING_KV").is_some() {
            ...
            eprintln!(
                ...
                bc = self.cell_sub.bin_count,
                bpm = self.cell_sub.bin_population_max,
                btt = ms(self.cell_sub.bin_task_elapsed_total),
                btm = ms(self.cell_sub.bin_task_elapsed_max),
                dd = ms(self.dedup),
